...
class TestSpotify(Base):
    def test_getArtistTopTrack(self):
        try:
            ...
            for artist_id in artist_ids:
                ...
                try:
                    rdx_json = self.get_rdx_artist_top_tracks(artist_id)
                    native_json = self.get_native_artist_top_tracks(artist_id, access_token)
            ...
            print('Exception in test_top_artist.py')
            assert True == False
    ...
    def get_rdx_artist_top_tracks(self, artist_id):
        url = getConfigration()['SPOTIFY']['api_endpoint'] + APIResources.rdx_Artist_top_track.format(id=artist_id)
        print(f'rdx_url={url}')
        headers = {'x-api-key': 'NWolYdwUTffmMjCcCiVdJKraDZfxYl'}
        response = self.getResponse(url, headers, 'API')
        return response

    def get_native_artist_top_tracks(self, artist_id, access_token):
